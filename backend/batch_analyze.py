...
# Import SessionLocal and Word for database persistence
from ingest_db import SessionLocal, Word
...
def bulk_update_words(db_updates: list):
    """
    Perform bulk update of words table with calculated cryptographic values.
    """
    with SessionLocal() as session:
        session.bulk_update_mappings(Word, db_updates)
        session.commit()

async def process_chapter(client: httpx.AsyncClient, book_osis: str, chapter_num: int, output_dir: str) -> str:
    """
    Fetch a chapter, process all its words, write results to file, and persist to database.
    """
    try:
        ...
        # Write results to output file and compile database mappings
            ...
            for verse in verses:
                ...
                for word in verse.get('words', []):
                    ...
                    if res:
                        ...
                        # Eagerly prepare database update structure using word id
                        if 'id' in word:
                            db_updates.append({
                                "id": word['id'],
                                "gematria_absolute": res['gematria_absolute'],
                                "gematria_ordinal": res['gematria_ordinal'],
                                "gematria_reduced": res['gematria_reduced'],
                                "atbash": res['atbash'],
                                "albam": res['albam'],
                                "atbah": res['atbah']
                            })
        ...
        # Bulk persist updates to PostgreSQL via synchronous connection pool in a thread
        if db_updates:
            await asyncio.to_thread(bulk_update_words, db_updates)
...
async def run_batch_analysis():
    ...
    print("Starting batch cryptographic analysis and database seeding...")
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        for idx, book in enumerate(books, 1):
            ...
            while True:
                ...
                sys.stdout.write(f"\r  Processing and Seeding Chapter {chapter_num}...")
                sys.stdout.flush()
                
                status = await process_chapter(client, book_osis, chapter_num, book_dir)
                if status == "EOF":
                    sys.stdout.write(f"\r  Finished all chapters and database seeding for {book_osis}.\n")
                    break
                elif status == "SUCCESS":
                    success_chapters += 1
                else:
        ...
        print("\n" + "="*50)
        print("Batch Cryptographic Analysis & Database Seeding Complete!")
